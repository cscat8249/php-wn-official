...
add_stylesheet('<link rel="stylesheet" href="'.$board_skin_url.'/fresco.css">', 1);
?>
<script type="text/javascript" src="<?=$board_skin_url?>/js/fresco.js"></script>
...
<div id="bo_gall" style="width:<?php echo $width; ?>">
    ...
    <ul id="gall_ul">
        ...
        <li class="gall_li <?php if ($wr_id == $list[$i]['wr_id']) { ?>gall_now<?php } ?>" style="<?php echo $style ?>width:<?php echo $board['bo_gallery_width'] ?>px">
            ...
            <?php } ?>
			<?if($is_admin){?><a href="/bbs/write.php?w=u&bo_table=<?=$bo_table?>&wr_id=<?=$list[$i]["wr_id"]?>&page=<?=$page?>">[수정]</a><?}?>
...
?>				<a href="<?=$big_img_url?>" data-fresco-caption="<?php echo $list[$i]['subject'] ?>" data-fresco-group='example' class='fresco'>
				  <?
					if($thumb['src']) {
